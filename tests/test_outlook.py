...
def test_build_picks_the_slot_and_words(client):
    ...
    assert "Thunderstorms, high near 96F, low around 78F, 60% chance of rain, wind up to 24 mph." in body
    ...
    assert ol.push_text(t) == ("Today's outlook · Thu Sep 10", "Mostly clear, 104/81, 5% rain")
...
def test_payload_summary_and_key(client):
    ...
    assert rp.outlook_summary(p) == "Thunderstorms, 96/78, 60% rain"
